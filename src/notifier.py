"""Notifications via the Telegram Bot API.

Uses the stdlib urllib (no extra dependencies). For confirmations it uses an
inline keyboard + getUpdates polling with an offset, no webhook.
"""
...
def _post_multipart(url: str, fields: dict, files: dict[str, Path]) -> dict:
    """POST multipart/form-data for file upload (sendPhoto)."""
...
def answer_callback(cfg: Config, callback_id: str, text: str = "") -> None:
    try:
        ...
        log.warning("answerCallbackQuery failed (non-blocking): %s", e)


def send_purchase_request(cfg: Config, screenshot: Path, caption: str, token: str) -> int:
    """Send a photo with Confirm/Cancel buttons. `token` distinguishes this request from previous ones."""
    keyboard = {
        "inline_keyboard": [[
            {"text": "✅ Confirm purchase", "callback_data": f"buy:confirm:{token}"},
            {"text": "❌ Cancel", "callback_data": f"buy:cancel:{token}"},
        ]]
    }
...
def wait_for_callback(cfg: Config, token: str) -> str:
    """Poll getUpdates until a callback with the expected token arrives.
    ...
    log.info("Waiting for Telegram callback (token=%s, timeout=%sh)", token, cfg.confirm_timeout_hours)
    ...
    while time.monotonic() < deadline:
        ...
        for update in updates:
            ...
            try:
                ...
                continue
            if recv_token != token:
                answer_callback(cfg, cb["id"], "Invalid or expired request.")
                continue
            if action == "confirm":
                answer_callback(cfg, cb["id"], "Confirmed.")
                return "confirm"
            if action == "cancel":
                answer_callback(cfg, cb["id"], "Cancelled.")
                return "cancel"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test", action="store_true", help="Send a test message and wait for a callback for 60s")
    parser.add_argument("--get-chat-id", action="store_true", help="Print the updates received by the bot to discover your chat_id")
    args = parser.parse_args()

    cfg = load_config()

    if args.get_chat_id:
        # To discover the chat_id: start the bot by sending it /start on Telegram,
        # then run this command.
        result = _post_json(_api_url(cfg.telegram_bot_token, "getUpdates"), {})
        if not result:
            print("No updates. Go to Telegram, find your bot, send it /start and try again.")
        else:
            for upd in result:
        ...
        send_message(cfg, "<b>Test notifier</b>\nPress a button within 60s.", reply_markup=keyboard)
        # Override the timeout for the test
